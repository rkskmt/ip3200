...
- たくさんの値を **区間ごとに数える** グラフ
- 平均だけでは分からない **ばらつき・偏り・山の形** が見える
- `bins` で区間の数を指定する
- `plt.axvline(値)` で **縦線** を引ける
- `np.median(x)` で **中央値** を計算できる
- 年収のように一部の値が大きいデータでは、平均と中央値がずれる
...
::: {.callout-note}
## 平均と中央値
- 一部の大きな値があると、平均は **右に引っ張られる**
- 中央値は **真ん中の人** に近い値を示す
- 分布が左右対称でないときは、平均だけでは代表値として弱い
- `plt.axvline(線を引く位置, label="説明")` で凡例つきの縦線を描ける
:::
